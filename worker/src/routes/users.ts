...
// Helper to get settings store
async function getSettingsStore(c: any): Promise<BlobSettingsStore> {
  if (!c.settingsStore) {
    const { createSettingsStore } = await import("../db/settings-blob");
...
userRoutes.get("/:username/settings/:key", authRequired, async (c) => {
  ...
  const settingsStore = await getSettingsStore(c);
  const setting = await settingsStore.getUserSetting(user.id, key);
...
userRoutes.patch("/:username/settings/:key", authRequired, async (c) => {
  ...
  const settingsStore = await getSettingsStore(c);
  const existing = await settingsStore.getUserSetting(user.id, key);
  let merged = newValue;
  if (existing) {
    try {
  ...
  await settingsStore.setUserSetting(user.id, key, valueStr);
...
userRoutes.get("/:username/personalAccessTokens", authRequired, async (c) => {
  ...
  const settingsStore = await getSettingsStore(c);
  const setting = await settingsStore.getUserSetting(user.id, "personal_access_tokens");
...
userRoutes.post("/:username/personalAccessTokens", authRequired, async (c) => {
  ...
  await settingsStore.setUserSetting(user.id, "personal_access_tokens", JSON.stringify(tokens));
